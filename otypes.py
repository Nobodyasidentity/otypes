...
class ostr(metaclass=_.abc.ABCMeta):
    ...
    def __new__(cls: _.Type[T],*parts:object,sep: _.Optional[str]=" ")->"ostr.ostr":
        ...
        if sep is None:sep=""
        inner=getattr(cls,"ostr")
        text=sep.join(str(p)for p in parts_iter)
        return inner(text)
    class ostr(str):
        """Concrete str subclass created by the factory."""
        __name__=__qualname__="ostr"
        ...
        def setitem(self, key, value):
            ...
            if isinstance(key,int):
                if key<0:key+=len(s)
                new=s[:key]+str(value)+s[key+1:]
                return type(self)(new)
            return type(self)(s)
        def __reduce__(self):return(ostr.ostr,(str(self),))
        snake=property(lambda s:(lambda t:t if t not in{"con","prn","aux","nul"}else f"{t}_")(_.re.sub(r"_+","_",_.re.sub(r"[^\w]+","_",_.unicodedata.normalize("NFKD",str(s)).encode("ascii","ignore").decode().strip().lower(),),).strip("_")[:255]or"unnamed"))
    ...
    method=staticmethod(lambda name=None,func=None,*,prop=False:((lambda f:(setattr(ostr.ostr,name or f.__name__,property(f)if prop else f)or f))if func is None else(setattr(ostr.ostr, name, property(func) if prop else func) or func)))
    @staticmethod
    def methods()->_.Dict[str, object]:return{name:value for name,value in _.inspect.getmembers(ostr.ostr)}
